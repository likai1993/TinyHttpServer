...
// -----------------------------------------------------------------------------
// Given an attribute, extract its value from the request header 
// -----------------------------------------------------------------------------

void get_header_by_name(const http_request_t req, const std::string attribute, std::string& ret)
{
    for (auto e : req.get_header()){
	std::string key = e.substr(0, e.find(":"));
	if (attribute.compare(key) == 0 ){
		ret = e.substr(e.find(":")+1, e.length()+1);
		break;
	}
    }
}
...
http_response_t::http_response_t(
    const http_request_t& request, const std::string& web_root, bool redirect_mode, const std::string& backend_server_addr)
{
	...
	auto it = _mime_tbl.find(mimekey);
		...
		if (gen_utils::compare_time(file_t, req_t))
		{
        		response_head = "HTTP/1.1 304 Not Modified\r\n";
			set_status_code(304);
		}else{
...
private:
    ...
    inline bool redirect_mode() const { return _redirect_mode; }
    ...
    inline http_server_task_t(bool verbose_mode, std::ostream& logger,
        tcp_socket_t::handle_t socket_handle, const std::string& web_root, bool redirect_mode, const std::string& backend_server_addr)
        : _verbose_mode(verbose_mode)
        , _logger(logger)
        , _tcp_socket_handle(socket_handle)
        , _web_root(web_root)
        , _redirect_mode(redirect_mode)
        , _backend_server_addr(backend_server_addr)
    {
    }
...
public:
        ...
        tcp_socket_t::handle_t socket_handle, const std::string& web_root, bool redirect_mode, const std::string& backend_server_addr)
    {
        return handle_t(new http_server_task_t(
            verbose_mode, logger, socket_handle, web_root, redirect_mode, backend_server_addr));
    }
...
void http_server_task_t::operator()(handle_t task_handle)
{
    ...
    while (get_tcp_socket_handle()) {
        ...
        http_response_t response(*http_request, web_root_dir(), redirect_mode(), get_backend_server_addr());
...
bool http_server_t::bind(std::string addr, port_t port)
{
    _tcp_server = tcp_listener_t::create();
...
bool http_server_t::run()
{
    ...
    while (true) {
        const tcp_socket_t::handle_t handler = accept();
        ...
        std::thread worker_thread(*task_handler, task_handler);
	
	std::cout<< "current worker thread ID :" << worker_thread.get_id() << std::endl;
